                            ...
                            <div class="row w-100 d-flex justify-content-between">
                                ...
                                <div class="col-3">
                                    <button wire:click="clearCart()"
                                        class="btn btn-danger">{{ __('Empty Cart') }}</button>
                                </div>
                            </div>
                            <hr>
                            {{-- cart items --}}
                                        ...
                                        <div class="row bg-white">
                                            <div class="col-md-3 pl-0">
                                                <img src="{{ $item->options['cover'] }}"
                                                    style="height: 200px; width:250px" alt="Image1"
                                                    class="img-fluid">
                                            </div>
                                            <div class="col-md-6 mt-4">
                                                <h5 class="pt-2">{{ $item->name }}</h5>
                                                <small class="text-secondary">{{ __('seller') }}: Momtaz
                                                    Nussair</small>
                                                <h5 class="pt-2">{{ '$' . $item->price }}</h5>
                                                <button type="submit"
                                                    class="btn btn-warning">{{ __('Save for later') }}</button>
                                                <button wire:click='removeItem("{{ $item->rowId }}")'
                                                    class="btn btn-danger mx-2"
                                                    name="remove">{{ __('Remove') }}</button>
                                            </div>
                                            <div class="col-md-3 py-5">
                                                <div>
                                                    <button
                                                        wire:click='updateQty("{{ $item->rowId }}", "{{ $item->qty - 1 }}")'
                                                        type="button" class="btn bg-light border p-2 rounded-pill"><i
                    ...
                    <div class="col-md-4 border rounded mt-5 bg-white h-25">
                        ...
                        <div class="pt-4">
                            ...
                            <form @if ($total <= 0) wire:submit.prevent="emptyCart" @endif action="{{ route('pay') }}" method="POST">
                                ...
                                @include("user.payment-methods." . env('PAYMENT_SERVICE'))
                                
                                <div class="row">
                                    <button @if ($total <= 0) disabled  @endif type="submit"
                                        class="btn btn-warning mt-4 w-100">{{ __('Proceed to checkout') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
